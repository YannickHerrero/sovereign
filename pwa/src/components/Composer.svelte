  ...
  <div class="overlay">
    ...
    <div class="sheet" style:bottom="calc(var(--safe-bottom) + 12px + {keyboard.height}px)">
      ...
      {#if c.missingReferences.length}
        <div class="voice-error" role="alert">Missing attachments: {c.missingReferences.join(', ')}. Remove these references before sending.</div>
      {/if}
      ...
      <div class="bar">
        ...
        {:else if c.hasDraft}
          <button class="round send" aria-label="Send" disabled={c.busy || modelBusy || c.missingReferences.length > 0} onclick={submit}>
