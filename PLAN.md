...
État au 10 septembre 2026 : F1, F2, F3a et F3b livrées. F1 vérifiée par huit contrôles Playwright (frappe capturée, Shift accepté, Ctrl ignoré, champ de recherche et dialogue et menu non capturés, nouvelle tâche). F2 vérifiée par tests unitaires et captures desktop et mobile. F3 vérifiée sur un diff réel à trois fichiers : arbre, unifié, côte à côte, dépliage de contexte avec numéros de ligne corrects, coche Viewed reflétée dans l'arbre, Échap qui revient à la conversation ; le mobile garde sa feuille. F3c livrée le même jour : highlight.js chargé à la demande (cœur 21 Ko, une grammaire de 2 à 8 Ko par langage rencontré, 25 langages déclarés explicitement pour éviter les 190 chunks), coloration ligne par ligne, palette discrète compatible avec les teintes d'ajout et de suppression. Ajout hors plan : mode pleine largeur du panneau de conversation (menu "…", mémorisé), à la manière de Notion. Chargement paresseux renforcé : la vue charge d'abord les statistiques (`?summary=true`), puis chaque patch à l'approche de son bloc (`?path=`), et un fichier de plus de 1 500 lignes de diff attend un clic "Show diff" avant de se rendre.
